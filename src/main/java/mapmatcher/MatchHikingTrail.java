...
public class MatchHikingTrail {
    ...
    public static void main(String[] args) throws IOException {
        ...
        bw.write("idgenerique idptini idsentier x y\n");
        ...
        // L'objet inséré à la fin dans outfile : 
...
//      ftVector.addFeatureAttribute(new AttributeType("id", "int"));
        ...
        for (int i = 0; i < gpsPop.size(); i++) {
            ...
            Arc edge = popMatchedVectors.nouvelElement(line); //popMatchedVectors
            edge.setId(i);
            double length = line.length();
            edge.setPoids(length);
            lengths.add(length);
            
            // Ecriture du fichier des points matchés
            IPoint pmatche = (IPoint) popMatchedPoints.get(i).getGeom();
            bw.write(i
                + " " + gpsPop.get(i).getAttribute("idp")
                + " " + result.getStates().get(i).getCorrespondant(0).getAttribute("SL_ID")
                + " " + pmatche.getPosition().getX() 
                + " " + pmatche.getPosition().getY() + "\n");
        }
